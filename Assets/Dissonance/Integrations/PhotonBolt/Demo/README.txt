Dissonance Photon Bolt Demo
===========================

This demo demonstrates the core features of Dissonance voice comms with Photon Bolt networking:

* The Dissonance Photon Bolt "DissonanceSetup" prefab.
...
1. Add both "Bolt Demo" and "Bolt Game World" scenes to your project's build settings, and drag "Bolt Demo" to the top of the list.
2. Go to Bolt->Assets, and define a new State:
    * Name the new state "DissonancePlayerState", and give two properties: a transform called "Transform", and a string called "DissonancePlayerId". See: BoltStateSettings.png
    * Ensure "IDissonancePlayerState" is selected as the state in the bolt entity script for the player prefab at `Dissonance/Integrations/PhotonBolt/Demo/Prefabs/Bolt Player Prefab`.
3. Add two new Events (both with no properties):
	* DissonanceToClient
	* DissonanceToServer
4. Click Bolt -> Compile Assembly
5. Click File -> Build and Run.
6. Once the client is running, run the Demo scene in the editor.
7. In editor, click "Start Server".
8. On the client, click "Start Client".
9. Press tab to toggle the Photon Bolt log.

The demo scene should load on both instances with both players connected. Use WASD to move the player.
